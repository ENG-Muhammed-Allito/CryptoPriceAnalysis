...
class KarmaDTRModel:
    # Karma veri seti için Decision Tree Regressor modeli
    ...
    def __init__(self, interval):
        ...
        self.model_dir = f"d:/Crypto_Analysis_random_state/karma_models/dtr/{self.interval.lower()}"
    ...
    def prepare_data(self):
        # Veriyi hazırlar ve eğitim/test kümelerine böler
        print(f"\n{self.interval.capitalize()} DTR modeli için veriler hazırlanıyor...")
    ...
    def train(self):
        # Modeli eğit
        print(f"\n{self.interval.capitalize()} DTR modeli eğitiliyor...")
    ...
    def _print_results(self, performance):
        # Performans ölçümlerini göster
        print("\n" + "=" * 50)
        print(f"Karma {self.interval.capitalize()} DTR modeli sonuçları")
        print("=" * 50)
        ...
        print(f"▪ Özellik önemi:")
        
        # Özellik önemini göster
        feature_importance = sorted(zip(performance['features'], self.model.feature_importances_), 
                                   key=lambda x: x[1], reverse=True)
        for feature, importance in feature_importance[:5]:  # En önemli 5 özelliği göster
            print(f"  - {feature}: {importance:.4f}")
        
        print()

def train_all_intervals():
    # Tüm zaman aralıkları için DTR modellerini eğit
    intervals = ['daily', 'hourly', 'monthly']
    results = {}
    
    for interval in intervals:
        try:
            model = KarmaDTRModel(interval)
    ...
    print("\nKarma DTR modelleri eğitimi tamamlandı!")
